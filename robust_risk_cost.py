...
for u in range(2,3):
    ...
    Risk=M.addVar(vtype=gp.GRB.CONTINUOUS,name='Risk')
    ...
    #添加偏差变量
    e_risk=M.addVar(vtype=gp.GRB.CONTINUOUS,name='e_risk')
    e_cost=M.addVar(vtype=gp.GRB.CONTINUOUS,name='e_cost')
    ...
    #风险最小化加鲁棒
    M.addConstr(Risk==gp.quicksum(p[c]*H[c] for c in scenario)+ 
                   tau*gp.quicksum(p[c]*(H[c]-gp.quicksum(p[c]*H[c] for c in scenario)
                    +2*delta_risk[c]) for c in scenario))
    
    #双目标转单目标
    M.setObjective((risk_lambda[u-1]*e_risk)/k_risk+(cost_lambda[u-1]*e_cost)/k_cost,sense=gp.GRB.MINIMIZE)
    #--------------------------------------------------
    #添加约束条件
    #双目标增添的约束条件
    M.addConstr(Risk-e_risk==Risk_min)
    M.addConstr(Cost-e_cost==Cost_min)
    ...
    str1='50_('+'R_'+str(risk_lambda[u-1])+'-'+'C_'+str(cost_lambda[u-1])+').lp'
    str2='50_('+'R_'+str(risk_lambda[u-1])+'-'+'C_'+str(cost_lambda[u-1])+').json'
    str3='50_('+'R_'+str(risk_lambda[u-1])+'-'+'C_'+str(cost_lambda[u-1])+').attr'
    #输出该模型求解的各种信息
    M.write(str1)
    M.write(str2)
    M.write(str3)
    ...
    str4='D:\\gruobi练习\\\''+'输出数据双目标50_'+str(u)+'.xlsx'
    book.save(str4)
